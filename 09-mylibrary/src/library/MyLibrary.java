...
public class MyLibrary {
	...
	public String getBooksCatalog(Book[] results) {
	...
	public String getBooksCatalog(Book[] results, Comparator<Book> comp) {
		if(comp != null) {
...
//		}
		sb.append(String.format("|%-4s|%-30.30s|%-20.20s|%-6s|%-15.15s|%-15.15s|%-15.15s|\n", 
				" ID", "       Title", "     Authors", " Date", "    Publisher", "  Genre", 
				"  Keywords"));
		
		sb.append(getUnderline(108)).append("\n");
		
		for(Book b : results) {
			sb.append(String.format("|%4d|%-30.30s|%-20.20s|%6tY|%-15.15s|%-15.15s|%-15.15s|", 
					b.getId(), b.getTitle(), b.getAuthors(), b.getPublishedDate(), b.getPublisher(), 
					b.getGenre(), b.getKeywords()))
			.append("\n");
		}
